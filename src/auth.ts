...
/**
 * Verify a bearer token. If `namespace` is given, also checks that the token
 * owns that namespace (returns `nsId` in the result). If omitted, just
 * resolves token ownership (used for endpoints like `POST /namespaces` where
 * the namespace doesn't exist yet).
 *
 * When `isLocal` is true, the well-known `LOCAL_TOKEN` is accepted with no
 * limits.
 */
export async function verifyToken(
  db: Database,
  token: string,
  options: { namespace?: string; isLocal?: boolean } = {},
): Promise<AuthResult | null> {
  const { namespace, isLocal = false } = options;
  ...
  if (isLocal && token === LOCAL_TOKEN) {
    ...
    return { tokenId: 0, nsId: 0, legendumToken: null };
  }
  ...
  if (!row) return null;
  return { tokenId: row.id, nsId: 0, legendumToken: row.legendum_token };
}
