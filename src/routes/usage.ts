...
export function handleGetUsage(
  ...
  const totalEvents = db
    .query(
      `SELECT COUNT(*) as c FROM events WHERE ns_id = ? AND created_at >= datetime('now', 'start of month')`,
    )
    .get(nsId) as { c: number };
  ...
  const webhookDeliveries = db
    .query(
      `SELECT COUNT(*) as c FROM notification_rules WHERE ns_id = ? AND url IS NOT NULL AND last_fired_at >= datetime('now', 'start of month')`,
    )
    .get(nsId) as { c: number };

  const emailsSent = db
    .query(
      `SELECT COUNT(*) as c FROM notification_rules WHERE ns_id = ? AND email IS NOT NULL AND last_fired_at >= datetime('now', 'start of month')`,
    )
    .get(nsId) as { c: number };
